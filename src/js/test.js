...
function markUp(data) {
  ...
  const str = `<div class="modal-film-item" id="${id}" >
                            ...
                            <img class="modal-film-item-img" src="${BASE_IMG_URL_w500}${poster_path}" alt="${original_title}" loading="lazy">
                            </div>
                            <div class="modal-film-item-title">
                                <h3 class="modal-film-item-main-h3 ${whiteThemeText}">${title}</h3>
                                <p class="modal-film-item-h3 ${whiteThemeText}">Vote / Votes<span class="modal-film-vote ${whiteThemeText}">${vote_average}</span> / <span class="modal-film-votes ${whiteThemeText}">${vote_count}</span></p>
                                <p class="modal-film-item-h3 ${whiteThemeText}">Popularity<span class="modal-film-popular ${whiteThemeText}">${parseFloat(popularity).toFixed(1)}</span></p>
                                <p class="modal-film-item-h3 ${whiteThemeText}">Genre<span class="modal-film-genre ${whiteThemeText}">${genres.map(({ name }) => name).join(', ')}</span></p>
                      <p class="modal-film-item-p ${whiteThemeText}">About</p><span class="modal-film-text ${whiteThemeSecondaryText}">${overview}</span>
                      <button type="submit" class="modal-film-btn button-card-modal ${whiteThemeBtn}">${textBtn(id)}</button>
                  </div>
                </div>
`;
